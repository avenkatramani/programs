...
import ctypes, os, time, struct, numpy, sys, threading, scipy.io, Queue, readline, gc, multiprocessing, multiprocessing.sharedctypes
...
class HRMTime(LabradServer):
    ...
    def acquire(self,c,save_prefix,runtime):
        ...
        #SAVE_PREFIX = 'test'
        self.acq.save_prefix = save_prefix
        
        #counter_start =0
        #counter_max =-1    # set to < 0 to keep going forever
        self.acq.target_filesize_MB = 2
        #max_filetime = 50 # maximum time to wait for one file, in seconds
        
        self.acq.process = True # whether to also process the clicks into cycles and gates 
        self.acq.save_old_format = False #whether to call HRM convertToCSV library to save files in old format
        
        #self.gate_time_us = 40
        #self.cycle_time_ms = 350
        
        self.acq.process_params = {}
        self.acq.process_params['gate_time_us'] = 40
        self.acq.process_params['cycle_time_ms'] = 350
        self.acq.process_params['min_gates'] = 10 # 10 for usual multi-gate cycles, set to 0 for single gate operation

        
        ##############################################
        
        self.acq.target_filesize = self.acq.target_filesize_MB*2**20
        
        

        print "Starting acquisition to %s for %d s..." % (save_prefix, self.acq.runtime)
        
        self.subproc_running = multiprocessing.sharedctypes.Value(ctypes.c_bool,True,lock=False)
        
        #self.thd_acq = threading.Thread(target=self._acquire)
        self.thd_acq = multiprocessing.Process(target=self.acq._acquire,args=(self.subproc_running,))
